...
Model klasifikasi dilatih dengan menggunakan dataset terbuka dari \textit{MobiAct Dataset} \citep{vavoulas-2016}. Dataset tersebut dikumpulkan dari 57 parisipan (42 laki-laki dan 15 perempuan) dalam rentang usia antara 20 sampai 47 tahun, rentang tinggi badan antara 160 cm sampai 189 cm dan rentang berat badan antara 50 kg sampai 120kg.

Data sensor akselerometer dan giroskop pada dataset tersebut diambil dengan \textit{sampling rate} tertinggi dari ponsel cerdas Android. Data tersebut di sample ulang dengan \textit{sampling rate} 50 Hz, lalu \textit{sliding window} dibuat dari setiap 100 sample dengan tumpang tindih 50\%. Dengan aturan tersebut, diperoleh data aktivitas seperti pada Tabel~\ref{table:jumlah-dataset}.
...
\begin{table}[h!]
    ...
    \begin{tabular}{ |p{0.5cm}|p{5cm}|p{7.5cm}| }
        ...
        \hline
        1 & Pengujian akurasi klasifikasi \textit{offline} & Didapatkan tingkat akurasi \dots pada data uji dari semua partisipan dan tingkat akurasi \dots pada data uji dengan partisipan yang terpisah dari data latih \\
